  ...
  return <IonModal isOpen={open} onDidDismiss={onClose} initialBreakpoint={0.9} breakpoints={[0, 0.5, 0.9]} className="live-sheet">
    <IonContent><div className="live-sheet__inner">
      ...
      {creating ? <div className="live-form">
        ...
        <div className="live-filter"><strong>Groupe musculaire</strong><div className="live-chips"><button className={muscle === null ? 'active' : ''} onClick={() => setMuscle(null)}>Tous</button>{groups.map((group, index) => <button key={group} className={muscle === index ? 'active' : ''} onClick={() => setMuscle(index)}>{group}</button>)}</div></div>
        <div className="live-filter"><strong>Équipement</strong><div className="live-chips"><button className={equipment === null ? 'active' : ''} onClick={() => setEquipment(null)}>Tous</button>{equipmentOptions.map(item => <button key={item} className={equipment === item ? 'active' : ''} onClick={() => setEquipment(item)}>{item}</button>)}</div></div>
        <p>{matches.length} exercice{matches.length > 1 ? 's' : ''} trouvé{matches.length > 1 ? 's' : ''}</p>
        {isPending && !data.length ? <p role="status">Chargement des exercices…</p> : error && !data.length ? <V5State title="Catalogue indisponible" error onRetry={() => void refetch()} /> : <div className="live-catalog-results">{matches.slice(0, 100).map(item => <button key={item.id} type="button" onClick={() => onSelect(item)}><span className="live-catalog-icon">✦</span><span><strong>{item.name}</strong><small>{item.muscleGroups?.map(group => groups[group]).join(' · ')}{item.equipment ? ` · ${item.equipment}` : ''}</small></span><span>＋</span></button>)}{matches.length > 100 && <p>Affichage des 100 premiers résultats. Affine ta recherche.</p>}</div>}
