...
- **Avancement (orchestration jev-orchestrator, `Docs/migration-ionic/plan-4g.md`)** :
  ...
  - **Plein écran natif (2026-09-26)** : fond Paragon posé sur `html`/`body`/`ion-app`/`.ion-page` (plus aucun blanc sous la barre d'état, pendant les transitions ou le rebond), `theme-color` et manifeste en `#1A7964` (haut du fond), barre d'onglets translucide floutée, vraie icône d'app (l'icône Blazor violette était restée) + icône *maskable* et `apple-touch-icon`, écrans de lancement iOS pour 11 tailles d'iPhone (`scripts/pwa-assets.mjs` les régénère), bandeau « Passe en plein écran » affiché une seule fois dans Safari sur iPhone.
