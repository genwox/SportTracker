...
- **Avancement (orchestration jev-orchestrator, `Docs/migration-ionic/plan-4g.md`)** :
  ...
  - **Vague 5 ✅ intégrée (2026-09-25)** : **L9** (`SportTracker.Web/Dockerfile` build Node → nginx statique, `nginx.conf` fallback SPA + `Cache-Control: no-cache` sur `index.html`/`service-worker.js`, service `web` dans `docker-compose.yml` sur le même modèle Traefik que `app`, host `beta.fmon-vps-n8n.fr`). **Le build Docker réel n'a pas pu être testé** (daemon Docker Desktop indisponible côté orchestrateur et côté worker) — seule la syntaxe (`docker compose config`) a été vérifiée. **À vérifier par Damien avant déploiement : `docker compose build web` puis test réel du conteneur.**
